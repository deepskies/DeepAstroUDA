...
Exact datasets (LSST mocks, SDSS wide and deep field and DeCALS data) we use in our work can be found on [Zenodo](https://doi.org/10.5281/zenodo.7473597).
...
| `deep_astro --config-path={path\to\referenced\file}` | Uses the provided path to get training configuration information from yaml. If file path not provided, a default training configuration yaml is created at the `DEFAULT_PATH="./deepastro_files/config_env/{dataset}_{domain}.yaml"` |
